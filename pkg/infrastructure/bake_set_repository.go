...
// FileBakeSetRepository ファイルベースのBakeSetRepository実装
type FileBakeSetRepository struct{}

// NewFileBakeSetRepository コンストラクタ
func NewFileBakeSetRepository() domain.BakeSetRepository {
	return &FileBakeSetRepository{}
}

// Save BakeSetのリストをJSONファイルに保存
func (r *FileBakeSetRepository) Save(bakeSets []*domain.BakeSet, filePath string) error {
	// ファイル拡張子の確認
...
func (r *FileBakeSetRepository) Load(filePath string) ([]*domain.BakeSet, error) {
	// ファイル読み込み
