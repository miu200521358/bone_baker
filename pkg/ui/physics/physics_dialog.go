...
func newPhysicsTableViewDialog(baseSet *entity.BakeSet, window *controller.ControlWindow) *PhysicsTableViewDialog {
...
func (p *PhysicsTableViewDialog) Show(record *entity.PhysicsRecord, recordIndex int) {
	...
	builder := declarative.NewBuilder(p.window)
...
func (p *PhysicsTableViewDialog) createFormWidgets(startFrameEdit, endFrameEdit,
	...
	return []declarative.Widget{
		...
		declarative.NumberEdit{
			...
			Increment:          1,
			MinValue:           0,
			MaxValue:           float64(p.baseSet.OriginalMotion.MaxFrame() + 1),
		},
		declarative.Label{
